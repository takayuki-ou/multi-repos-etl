...
def demo_filter_combination():
    ...
    try:
        ...
        # 3.5. ステータスフィルタのみ（リードタイム分析用）
        print("3.5. ステータスフィルタのみ（closed PRのみ）:")
        prs, error = data_manager.get_pull_requests_with_lead_time_data(
        ...
        if error:
            ...
            print(f"   取得件数: {len(prs)}件")
            for pr in prs[:3]:  # 最初の3件を表示
                print(f"   - PR#{pr['number']}: {pr['title']} ({pr['user_login']}) - {pr['state']}")
        print()
        ...
        if authors:
            author = authors[0]
            print(f"   条件: 日付範囲 (2023年1月-3月) AND 作成者 ({author}) AND ステータス (closed)")
            prs, error = data_manager.get_pull_requests_with_lead_time_data(
                repo_id, start_date=start_date, end_date=end_date, author=author, status='closed'
            )
            if error:
                print(f"   エラー: {error}")
            else:
        ...
        print(f"   無効なステータス: {is_valid}, エラー: {error}")
        
        # 有効なステータス（GitHubの仕様上、openとclosedの2択）
        for status in ['open', 'closed']:
            is_valid, error = data_manager.validate_filter_combination(status=status)
            print(f"   ステータス '{status}': {is_valid}, エラー: {error}")
